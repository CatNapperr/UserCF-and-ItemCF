...
class UserBasedCF(object):
    ...
    def cross_validate(self, filename, M=8, seed=0, csv_out='usercf_cv_results.csv'):
        '''
        ...
        # compute averages
        avg_precision = sum(r[1] for r in results) / M
        avg_recall = sum(r[2] for r in results) / M
        avg_coverage = sum(r[3] for r in results) / M
        avg_popularity = sum(r[4] for r in results) / M
...
if __name__ == '__main__':
    ...
    usercf = UserBasedCF()
    # Ensure K=20 similar users
    usercf.n_sim_user = 20
    # Run 8-fold cross validation and save results
    usercf.cross_validate(ratingfile, M=8, seed=0, csv_out='usercf_cv_results.csv')